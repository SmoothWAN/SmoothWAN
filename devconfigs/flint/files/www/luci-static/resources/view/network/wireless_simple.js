...
return view.extend({
	...
	renderOverview: function() {
		...
		s = m.section(form.GridSection, 'wifi-device', _('Wireless Overview'), _('Each radio is capable of joining a Wi-Fi network as a WAN while simultaneously broadcasting access point.'));
		s.anonymous = true;
		s.addremove = false;


		ms = m.section(form.TypedSection, 'wifi-iface', _('Wireless Setup'), _("- Wi-Fi channel follows wireless WAN if enabled. <br>- Click <b>Scan</b> button above to view available networks.<br>- Click <b>Enable</b> in the interfaces above after entering and applying wireless WAN settings.<br>- This configuration page is dumbed down & no automation to support various devices and hardware.<br>- Use WPA3 encryption in WAN section for iOS Hotspot."));
		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				...
				o = ss.taboption('general', widgets.NetworkSelect, 'network', _('Network'), _('Choose the network(s) you want to attach to this wireless interface or fill out the <em>custom</em> field to define a new network.<br><br><b>SmoothWAN Note!<br><br>If this is a Wireless WAN, head to Network->Interfaces and edit the automatically created interface to set the metric to a unique number (e.g 10 or more).</b>'));
				o.rmempty = true;
				o.multiple = true;
				o.novirtual = true;
		...
		s.handleScan = function(radioDev, ev) {
			...
			var md = ui.showModal(_('Wireless Scanner'), [
				table,
				E('div', { 'class': 'right' }, [
					stop,
					' ',
	...
	handleReset: function () {
		fs.exec_direct("/usr/bin/resetwifi").then(function () {
